  ...
  class Datatable
    ...
    def initialize(*args)
      if args.present? && args.first != nil
        ...
        elsif args.first.to_h.kind_of?(Hash)
          args.first.to_h.each { |k, v| self.attributes[k] = v }
        else
          raise "#{self.class.name}.new() can only be initialized with a Hash like arguments"
        end
      end
